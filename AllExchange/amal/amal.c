...
int main ( int argc , char * argv[] )
{
    ...
    fprintf( log , "\n    IDb (%lu Bytes):   ..... MATCH\n", (strlen(IDb) + 1));
    BIO_dump_indent_fp(log , IDb , (strlen(IDb) + 1) , 4);
    ...
    write_test = write(fd_A2B, &msg_len, len_len);

    if (write_test < len_len) {
        fprintf(log, "Unable to send MSG5 over A to B pipe.\n");
        fclose(log);
        exit(-1);
    }

    write_test = write(fd_A2B, msg5, msg_len);

    if (write_test < msg_len) {
        fprintf(log, "Unable to send MSG5 over A to B pipe.\n");
        fclose(log);
        exit(-1);
    }
