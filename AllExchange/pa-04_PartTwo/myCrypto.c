...
// Ks || L(IDb) || IDb  || Na || L(TktCipher) || TktCipher
    ...
    *lenTktCipher = *((size_t *)p);
    p += LENSIZE;

    *tktCipher = (uint8_t *) malloc(*lenTktCipher);
    if (!*tktCipher) {
        exitError("Memory allocation failed for TktCipher\n");
    }
...
size_t  MSG4_new( FILE *log , uint8_t **msg4, const myKey_t *Ks , Nonce_t *fNa2 , Nonce_t *Nb )
{
    ...
    if (sizeof(plaintext) < sizeof(**fNa2) + sizeof(**Nb)) {
        fprintf(log, "Failed to create MSG4. Plaintext buffer too small!\n");
        fclose(log);
        exit(-1);
    }
    ...
    **(((Nonce_t *) plaintext) + 1) = **Nb;
    ...
    if (LenMsg4 == 0 || LenMsg4 > sizeof(ciphertext)) {
        fprintf(log, "Failed to encrypt MSG4. Cipher text buffer may be too small.\n");
        fclose(log);
        exit(-1);
    }
    ...
    *msg4 = malloc(LenMsg4);
    ...
    if (*msg4 == NULL) {
        fprintf(log, "MSG4 malloc failed!\n");
        fclose(log);
        exit(-1);
    }

    // *(__uint128_t *) *msg4 = *((__uint128_t *) ciphertext); // Maybe use someting else.
...
void  MSG4_receive( FILE *log , int fd , const myKey_t *Ks , Nonce_t *rcvd_fNa2 , Nonce_t *Nb )
{
    ...
    ssize_t length;
    ssize_t read_test = 0;
    read_test = read(fd, &length, sizeof(ssize_t));

    if (read_test < sizeof(ssize_t)) {
        fprintf(log, "Failed to properly read from B to A pipe!\n");
        fclose(log);
        exit(-1);
    }

    read_test = read(fd, ciphertext, length);
    ...
    uint64_t decrypt_len =  decrypt(ciphertext, length, Ks -> key, Ks -> iv, plaintext);

    if (decrypt_len == 0 || decrypt_len > sizeof(plaintext)) {
        printf("Failed to decrypt. Plaintext buffer may be to small.\n");
        fclose(log);
        exit(-1);
    }

    **rcvd_fNa2 = **((Nonce_t *) plaintext);
    **Nb = **((Nonce_t *)(plaintext + sizeof(**rcvd_fNa2)));
...
size_t  MSG5_new( FILE *log , uint8_t **msg5, const myKey_t *Ks ,  Nonce_t *fNb )
{
    ...
    if (sizeof(plaintext) < sizeof(**fNb)) {
        fprintf(log, "Error: plaintext buffer too small.\n");
        fclose(log);
        exit(-1);
    }      

    **((Nonce_t *) plaintext) = **fNb;
    ...
    if (LenMSG5cipher == 0 || LenMSG5cipher > sizeof(ciphertext)) {
        fprintf(log, "Error: ciphertext buffer too small.\n");
        fclose(log);
        exit(-1);
    }
    ...
    *msg5 = malloc(LenMSG5cipher);
    // *((__uint128_t *) *msg5) = *((__uint128_t *) ciphertext); // Same as above.

    if (*msg5 == NULL) {
        fprintf(log, "msg5 failed to malloc!\n");
        fclose(log);
        exit(-1);
    }
...
void  MSG5_receive( FILE *log , int fd , const myKey_t *Ks , Nonce_t *fNb )
{
    ...
    read_test = read(fd, &LenMSG5cipher, sizeof(size_t));

    if (read_test < sizeof(size_t)) {
        fprintf(log, "Failed to read from A to B pipe.\n");
        exit(-1);
    }

    read_test = read(fd, ciphertext, LenMSG5cipher);

   if (read_test < LenMSG5cipher) {
        fprintf(log, "Failed to read from A to B pipe, or ciphertext buffer too small.\n");
        exit(-1);
    }
    ...
    if (dec_len == 0 || dec_len > sizeof(plaintext)) {
        fprintf(log, "Failed to decrypt, ciphertext buffer may be too small\n");
        exit(-1);
    }
